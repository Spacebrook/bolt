	...
	)
{
...
#if QUADTREE_QUERY_STATS == 1
	{
		...
		uint64_t nodes = 0;
		uint64_t entities = 0;
		quadtree_query_stats_get(&queries, &nodes, &entities);
		if(queries)
		{
			printf(
				"Query stats: nodes/query %.02f, entities/query %.02f\n",
				(double) nodes / (double) queries,
				(double) entities / (double) queries
				);
		}
	}
#endif
