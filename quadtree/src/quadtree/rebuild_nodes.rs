            ...
            if !is_leaf {
                ...
                if can_merge && total <= self.merge_threshold {
                    *did_merge = true;
                    ...
                    while current != 0 {
                        ...
                        let mut hash = (entity_idx as usize).wrapping_mul(2654435761) & merge_mask;
                        loop {
                ...
                if can_split {
                    ...
                    while node_entity_idx != 0 {
                        let next_node_entity_idx = old_node_entities_next[node_entity_idx as usize];
                        ...
                        if targets_len == 1 {
                            let child_half = Self::child_half_extent(info.half, targets[0]);
                        ...
                        if fast_no_dedupe {
                            ...
                            if total_count != 0 {
                                ...
                                if new_node_entities.capacity() < needed {
                                    new_node_entities
                                        .reserve(needed - new_node_entities.capacity());
                                }
                                if new_node_entities_next.capacity() < needed {
                                    new_node_entities_next
                                        .reserve(needed - new_node_entities_next.capacity());
                                }
                                ...
                                if new_node_entity_packed.len() < needed {
                                    new_node_entity_packed
                                        .resize(needed, NodeEntityPacked::default());
                                }
                                ...
                                let node_entities_values_ptr =
                                    new_node_entity_values.as_mut_ptr().add(start);
                                let node_entities_flags_ptr =
                                    new_node_entities_flags.as_mut_ptr().add(start);
                                let node_entities_packed_ptr =
                                ...
                                while offset < total_count {
                                    *node_entities_next_ptr.add(offset) =
                                ...
                                new_nodes[new_node_idx as usize]
                                    .set_dedupe_start(total_count as u32);
                                has_dedupe = false;
                            }
                        } else {
                            ...
                            while current != 0 {
                                let node_entity = *old_node_entities_ptr.add(current as usize);
                                let entity_idx = node_entity.index();
                                if large_entities_enabled
                                    && *large_entity_slots_ptr.add(entity_idx as usize) != 0
                                {
                                    current = *old_node_entities_next_ptr.add(current as usize);
                                    continue;
                                }
                                ...
                                while current != 0 {
                                    ...
                                    if !node_entity.has_dedupe() {
                                        ...
                                        if !large_entities_enabled
                                            || *large_entity_slots_ptr.add(entity_idx as usize) == 0
                                        {
                                            let in_nodes = (*entities_ptr.add(entity_idx as usize))
                                                .in_nodes_minus_one;
                                            let mapped_idx =
                                                mapper.map_entity(entity_idx, in_nodes);
                                            let out = write_offset;
                                            let mut new_entity = NodeEntity::new(mapped_idx);
                                            new_entity.set_dedupe(false);
                                            *node_entities_ptr.add(out) = new_entity;
                                            let extent =
                                                old_entity_extents.extent(entity_idx as usize);
                                            *node_entities_min_x_ptr.add(out) = extent.min_x;
                                            *node_entities_min_y_ptr.add(out) = extent.min_y;
                                            *node_entities_max_x_ptr.add(out) = extent.max_x;
                                            *node_entities_max_y_ptr.add(out) = extent.max_y;
                                            ...
                                            *node_entities_packed_ptr.add(out) =
                                                NodeEntityPacked::from_parts(
                                                    extent,
                                                    *entity_values
                                                        .get_unchecked(entity_idx as usize),
                                                    new_entity,
                                                );
                                            write_offset += 1;
                                        }
