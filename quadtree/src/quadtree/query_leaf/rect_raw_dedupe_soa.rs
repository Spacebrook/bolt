impl QuadTreeInner {
        ...
        F: FnMut(u32),
    {
        ...
        if use_avx2 {
            ...
            while idx + 8 <= end {
                ...
                if (mask & 0b0000_0001) != 0 {
                    let entity_idx = (*node_entities_ptr.add(base)).index() as usize;
                    if *query_marks_ptr.add(entity_idx) != tick {
                        *query_marks_ptr.add(entity_idx) = tick;
                        f(*node_entity_values_ptr.add(base));
                    }
                }
                if (mask & 0b0000_0010) != 0 {
                    let entity_idx = (*node_entities_ptr.add(base + 1)).index() as usize;
                    if *query_marks_ptr.add(entity_idx) != tick {
                        *query_marks_ptr.add(entity_idx) = tick;
                ...
                if (mask & 0b0000_0100) != 0 {
                    let entity_idx = (*node_entities_ptr.add(base + 2)).index() as usize;
                    if *query_marks_ptr.add(entity_idx) != tick {
                        *query_marks_ptr.add(entity_idx) = tick;
                ...
                if (mask & 0b0000_1000) != 0 {
                    let entity_idx = (*node_entities_ptr.add(base + 3)).index() as usize;
                    if *query_marks_ptr.add(entity_idx) != tick {
                        *query_marks_ptr.add(entity_idx) = tick;
                ...
                if (mask & 0b0001_0000) != 0 {
                    let entity_idx = (*node_entities_ptr.add(base + 4)).index() as usize;
                    if *query_marks_ptr.add(entity_idx) != tick {
                        *query_marks_ptr.add(entity_idx) = tick;
                ...
                if (mask & 0b0010_0000) != 0 {
                    let entity_idx = (*node_entities_ptr.add(base + 5)).index() as usize;
                    if *query_marks_ptr.add(entity_idx) != tick {
                        *query_marks_ptr.add(entity_idx) = tick;
                ...
                if (mask & 0b0100_0000) != 0 {
                    let entity_idx = (*node_entities_ptr.add(base + 6)).index() as usize;
                    if *query_marks_ptr.add(entity_idx) != tick {
                        *query_marks_ptr.add(entity_idx) = tick;
                ...
                if (mask & 0b1000_0000) != 0 {
                    let entity_idx = (*node_entities_ptr.add(base + 7)).index() as usize;
                    if *query_marks_ptr.add(entity_idx) != tick {
                        *query_marks_ptr.add(entity_idx) = tick;
        ...
        while idx < end {
            ...
            let entity_idx = (*node_entities_ptr.add(idx)).index() as usize;
            if *query_marks_ptr.add(entity_idx) != tick {
                *query_marks_ptr.add(entity_idx) = tick;
                f(*node_entity_values_ptr.add(idx));
            }
            idx += 1;
        }
    }
